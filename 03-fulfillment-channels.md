...
> **Status:** detailed spec (v0.1). Defines how the buyer's "where to fulfill" data is negotiated and exchanged.

## Why this is in the protocol, not an afterthought

Escrow exchanges separate payment (commit) from fulfillment (release). Some offers fulfill inline (the resource is the HTTP 200 body); others fulfill out-of-band (mailing list, license dispatch, package shipment, off-chain account provisioning). The buyer's required fulfillment data depends on three orthogonal choices:

- **Offer type** — inline / digital-non-atomic / physical.
...
Rather than fork that complexity into per-seller bespoke flows, the `escrow` scheme standardises a `FulfillmentChannel` interface. The 402 advertises which channels the seller supports; the buyer picks one and attaches channel-specific data alongside the payment payload (or out-of-band, depending on the channel).
...
For channels where data is collected post-commit (e.g. `widget`), `fulfillment.data` is `null` in the X-PAYMENT and the actual collection happens between commit and release via the channel's own mechanism.
...
export interface FulfillmentChannel<TServerCfg = unknown, TBuyerData = unknown> {
  ...
  /** Server: invoked when the release is observed; returns the resource (inline) or void (async). */
...
|---|---|---|---|
| `inline` | Resource returned in same HTTP response | `null` | Server `onFulfill` returns the body inline. Composes naturally with an atomic commit-and-release (the resource is ready), but commit-and-release does **not** require this channel — the release state transition can be atomic while fulfillment itself is async. |
| `email` | Mailing list signup, license key dispatch | `{ email: string }` | RFC 5321 validation. Server stores against exchangeId; sends on release. |
...
| `ipfs-pointer` | Server uploads to IPFS, returns CID | `{ recipientPubKey?: string }` | Optional encryption to recipientPubKey. Returned on release. |
| `widget` | Human buyer + physical goods (data collected by a widget post-commit) | `null` (collected by widget) | `metadata.widgetUrl` points the human to a redemption widget. |
...
The registry is open: third parties can ship additional channels and register them with the SDK at startup.
...
| `widget`, `mcp` | Post-commit, via the channel's mechanism | Heavier UI / agent-driven; not a fit for header-sized data. |

Servers SHOULD advertise at least one "in-payload" channel (email/xmtp/webhook) so headless agents can complete in a single round trip.
...
- For `webhook`, the buyer's `publicKey` MAY be used to encrypt the resource payload (out of scope for v1).
...
```ts
import { negotiateFulfillment } from "x402-escrow-fulfillment";
...
- **Multi-step fulfillments** (a license + an email confirmation): for v1, sellers split into separate offers. Multi-channel per offer is a v2 feature.
- **Data-at-rest encryption** for `email`/`webhook` storage on the server: out of scope; sellers handle per their stack.
- **Channel advertising in seller metadata** (so a buyer can pre-filter sellers by accepted fulfillment channels): implementation-specific.
